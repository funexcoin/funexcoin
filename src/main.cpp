...
#if defined(NDEBUG)
#error "Funexcoin cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 60 * 5;
int64_t nReserveBalance = 0;

/** Fees smaller than this (in uFUNX) are considered zero fee (for relaying and mining)
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    ret = blockValue * 0.60; // 66% of block reward
...
{
    RenameThread("Funexcoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // FUNX: We use certain sporks during IBD, so check to see if they are
